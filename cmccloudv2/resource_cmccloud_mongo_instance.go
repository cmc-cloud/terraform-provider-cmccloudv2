...
func resourceMongoInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceMongoInstanceCreate,
		Read:   resourceMongoInstanceRead,
		Update: resourceMongoInstanceUpdate,
		Delete: resourceMongoInstanceDelete,
		Importer: &schema.ResourceImporter{
			State: resourceMongoInstanceImport,
		},
		...
		SchemaVersion: 1,
		Schema:        mongoInstanceSchema(),
		CustomizeDiff: func(diff *schema.ResourceDiff, v interface{}) error {
			mode := strings.ToLower(diff.Get("mode").(string))
			secondaryQuantity := diff.Get("quantity_of_secondary").(int)
			// replica_set mode requires quantity_of_secondary
			if mode == "replica_set" {
				if secondaryQuantity <= 0 {
					return fmt.Errorf("`quantity_of_secondary` is required and must be > 0 when mode is `%s`", mode)
				}
			}
			return nil
		},
...
func resourceMongoInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	// #endregion
	...
	datastores, err := client.MongoInstance.ListDatastore(map[string]string{})
	if err != nil {
	...
	params := map[string]interface{}{
		...
		"subnetId":             subnet.ID,
		"quantityOfSecondary":  d.Get("quantity_of_secondary").(int),
	...
	switch mode {
	case "replica_set":
		requestMetadata["quantityOfSecondary"] = d.Get("quantity_of_secondary").(int)
...
func resourceMongoInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*CombinedConfig).goCMCClient()
...
func resourceMongoInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*CombinedConfig).goCMCClient()
...
func resourceMongoInstanceImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	err := resourceMongoInstanceRead(d, meta)
	return []*schema.ResourceData{d}, err
}

func waitUntilMongoInstanceJobFinished(d *schema.ResourceData, meta interface{}, timeout time.Duration) (interface{}, error) {
	return waitUntilResourceStatusChanged(d, meta, []string{"HEALTHY", "RUNNING"}, []string{"ERROR", "SHUTDOWN"}, WaitConf{
		Timeout:    timeout,
		Delay:      10 * time.Second,
		MinTimeout: 20 * time.Second,
	}, func(id string) (any, error) {
		return getClient(meta).MongoInstance.Get(id)
	}, func(obj interface{}) string {
		return strings.ToUpper(obj.(gocmcapiv2.MongoInstance).Status)
	})
}

func waitUntilMongoInstanceDeleted(d *schema.ResourceData, meta interface{}) (interface{}, error) {
